...
# --- Панель магазина в бою (дополнения) ---
SHOP_COFFEE_TEXT_X_OFFSET = 90          # Отступ текста с количеством кофе от левого края
SHOP_CARD_BORDER_RADIUS = 5             # Скругление рамки карточки в магазине
SHOP_BORDER_THICKNESS = 5               # Толщина внешней рамки панели магазина

# --- Сообщение о прохождении уровня ---
LEVEL_CLEAR_SHADOW_OFFSET = 5           # Смещение тени для текста "БРС: 100/100"

# --- Панель описания (дополнения) ---
DESC_PANEL_STATS_MAX_Y_OFFSET = 150     # Максимальное смещение по Y для списка статов, чтобы избежать выхода за пределы панели
DESC_PANEL_NO_STAT_TEXT = "Нет"         # Текст для отсутствующих характеристик
DESC_PANEL_RADIUS_UNIT = "кл."          # Единица измерения для радиуса
DESC_PANEL_SLOW_DURATION_UNIT = "сек."  # Единица измерения для длительности замедления
DESC_PANEL_COOLDOWN_UNIT = "сек."       # Единица измерения для перезарядки

# --- Расстановка нейросетей (дополнения) ---
PLACEMENT_MOWER_SLOT_BORDER_COLOR = (200, 200, 200) # Цвет рамки слота для нейросети
PLACEMENT_MOWER_CARD_IMG_PADDING = 10   # Отступ для изображения внутри перетаскиваемой карточки
