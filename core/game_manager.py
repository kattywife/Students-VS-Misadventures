...
class Game:
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Вызов метода-обработчика для текущего состояния
            handler = self.state_handlers.get(self.state)
            if handler:
                handler()
            else:
                # Обработка непредвиденного состояния
    ...
    def _prepare_level(self, level_id):
        ...
        Создает новый PrepManager и сбрасывает данные предыдущего уровня.

        Args:
            level_id (int): ID уровня, который нужно подготовить.
        """
    ...
    def _start_battle(self):
        ...
        Создает все необходимые группы спрайтов и менеджеры для нового боя.
        """
    ...
    def _start_screen_loop(self):
        """Игровой цикл для стартового экрана."""
    ...
    def _main_menu_loop(self):
        """Игровой цикл для главного меню."""
    ...
    def _settings_loop(self):
        """Игровой цикл для меню настроек."""
        self.screen.blit(self.background, (0, 0))
    ...
    def _preparation_loop(self):
        """Игровой цикл для экрана подготовки."""
        prep_manager = self.game_data['prep_manager']
    ...
    def _neuro_placement_loop(self):
        """Игровой цикл для экрана расстановки нейросетей."""
        prep_manager = self.game_data['prep_manager']
    ...
    def _handle_neuro_placement_events(self, event, unplaced_rects, start_rect):
        """Обрабатывает события на экране расстановки нейросетей (логика drag-and-drop)."""
            ...
            if dragged_mower is None:
                ...
                for index, rect in unplaced_rects.items():
                    if rect.collidepoint(pos):
                        ...
                        return
                # Попытка "взять" уже размещенную нейросеть из слота
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and dragged_mower:
            ...
            # Если "бросили" в зону размещения
    ...
    def _handle_level_win(self):
        ...
        # Разблокируем следующий уровень, если текущий был последним доступным
    ...
    def _level_clear_loop(self):
        """Промежуточный цикл после победы, перед показом меню победы."""
        ...
        # Ждем окончания звука победы, затем переходим к следующему состоянию
        ...
        # В это время на экране отрисовывается мир и сообщение о победе
                ...
                for text, rect in buttons.items():
                    if rect.collidepoint(event.pos):
                        ...
                        if next_states and text in next_states:
                            ...
                            if callable(action):
                                action()  # Выполняем лямбда-функцию
                            else:
                                self.state = action  # Меняем состояние
                        return

    def _paused_loop(self):
        """Цикл для состояния PAUSED."""
        next_states = {
    ...
    def _level_victory_loop(self):
        """Цикл для состояния LEVEL_VICTORY."""
        next_states = {
    ...
    def _game_victory_loop(self):
        """Цикл для состояния GAME_VICTORY."""
    ...
    def _go_to_main_menu(self):
        """Переходит в главное меню и включает соответствующую музыку."""
        self.sound_manager.stop_music()
        self.sound_manager.play_music('main_team')
        self.state = "MAIN_MENU"